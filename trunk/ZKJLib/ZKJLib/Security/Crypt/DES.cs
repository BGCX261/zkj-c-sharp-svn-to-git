...
namespace ZKJLib.Security.Crypt
{
    ...
    public class DES
    {
        ...
        public DES(byte[] bytesKey, byte[] bytesIV)
        {
            Key = bytesKey;
            IV = bytesIV;
        }
        ...
        public DES(byte[] bytesKey, byte[] bytesIV, Encoding encodingType)
        ...
        /// <param name="bytesIV">偏移向量</param>
        /// <param name="EncodingType">编码类型</param>
        public DES(string strKey, byte[] bytesIV,Encoding encodingType)
        {
            EncodingType = encodingType;
            Key = EncodingType.GetBytes(Commen.StdLen(strKey, 8));
            IV = bytesIV;
